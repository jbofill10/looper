...
// aborts one, h opens that loop's run history (viewRuns), R begins a
// rename, D begins a delete confirmation. All are no-ops when the cursor
// isn't on a loop row, and g/x are additionally no-ops when that loop has
// no active run.
