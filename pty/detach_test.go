...
func TestDetachScanner_CtrlBackslashThenD_Detaches(t *testing.T) {
	var d DetachScanner
	out, detached := d.Scan([]byte{0x1c, 'd'})
	if len(out) != 0 {
...
func TestDetachScanner_CtrlBackslashThenNonD_PassesThroughBoth(t *testing.T) {
	var d DetachScanner
	out, detached := d.Scan([]byte{0x1c, 'x'})
	want := []byte{0x1c, 'x'}
...
func TestDetachScanner_SplitAcrossCalls_Detaches(t *testing.T) {
	var d DetachScanner
	out1, detached1 := d.Scan([]byte{0x1c})
	if len(out1) != 0 {
...
func TestDetachScanner_DetachStopsConsumingRestOfInput(t *testing.T) {
	var d DetachScanner
	out, detached := d.Scan([]byte{0x1c, 'd', 'x', 'y'})
	if !detached {
